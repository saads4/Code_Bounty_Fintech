...
export default function Login({ onLoggedIn, onToast }){
  ...
  return (
    <div className="min-h-screen bg-[#1E1B3A] flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-[#A5B4FC] bg-[#2a2750] shadow-card p-6">
          ...
          <div className="text-xs text-[#E2E8F0] mt-3 text-center" style={{opacity:0.9}}>
            {mode==='login' ? (
              <>
                New here? <a className="text-[#C4B5FD] hover:underline" href="#" onClick={(e)=>{e.preventDefault(); setMode('register')}}>Create an account</a>
              </>
            ) : (
              <>
                Already have an account? <a className="text-[#C4B5FD] hover:underline" href="#" onClick={(e)=>{e.preventDefault(); setMode('login')}}>Sign in</a>
              </>
            )}
          </div>
        </div>
